    ...
    private void createFeatureTypesFile(final OutputStream outputStream, final XtraServerMapping xtraServerMapping) throws IOException {
        ...
        xtraServerMapping.getFeatureTypeMappings(true).forEach(featureTypeMapping -> {
            ...
            try {
                ...
                writer.append("</Name>\n\t\t");
                writer.append("{if {$featureTypes.");
		        writer.append(featureTypeNameWithoutPrefix);
                writer.append(".title}}<Title {if {$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".title.lang}}xml:lang=\"{$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".title.lang}\"{fi}>{$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".title}</Title>{fi}\n\t\t");
                writer.append("{if {$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".abstract}}<Abstract {if {$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".abstract.lang}}xml:lang=\"{$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".abstract.lang}\"{fi}>{$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".abstract}</Abstract>{fi}\n\t\t");
                writer.append("{if {$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".keywordList}}<Keywords>{forall kw {$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".keywordList}}\n\t\t\t");
                writer.append("<Keyword {if {$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".keywordList.lang}}xml:lang=\"{$featureTypes.");
                writer.append(featureTypeNameWithoutPrefix);
                writer.append(".keywordList.lang}\"{fi}>{$kw}</Keyword>{end}\n\t\t");
                writer.append("</Keywords>{fi}\n\t\t");
